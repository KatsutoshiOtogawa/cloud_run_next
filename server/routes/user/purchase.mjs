...
function purchaseRoute(router) {
  router.get("/purchase", csrfProtection, (req, res, next) => {
    ...
    session.username
    console.log(
      `Accessing the ${username} ${req.originalUrl} from ip address: ${req.ip} method: GET`
    );
    // ログインしてないならログイン画面に移す。
  ...
  router.post("/purchase", csrfProtection, (req, res, next) => {
    console.log(
